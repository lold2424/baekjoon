...
1. stack에 기본 문자열을 한글자씩 넣는다.
2. stack의 문자열의 길이가 폭발 문자열과 길거나 같으면 stack과 폭발 문자열과 비교한다.
3. 만약 같을경우 stack에서 폭발 문자열을 제거하고, 다르다면 그대로 남아있는다.
4. 모두 비교가 끝난 후 stack의 길이가 0보다 크다면 stack값을 0이라면 "FRULA"를 출력한다.

해당 문제는 수월하게 풀었다. 다만 마지막에 stack값을 StringBuilder에 넣는데 애를 먹었는데 인터넷에 검색해보니 향상된 for문을 이용하면 된다고하여 사용해 보았다.
