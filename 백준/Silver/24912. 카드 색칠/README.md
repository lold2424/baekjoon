...
처음에는 그냥 if문을 사용하여 입력받은 숫자가 0인지 아닌지를 분별하고 앞, 뒤 숫자에 맞춰 카드를 색칠하거나 올바른 값인지 확인하는 방식으로 문제를 풀었다.

하지만, if문을 너무 많이 사용하여 런타임 에러가 발생했고 if문을 압축하여 문제를 풀었다.

숫자를 입력받을때 정수로 입력받지않고 String형으로 입력받아 사용했고, 시간제한 때문에 StringBuilder에 값을 저장하여 한번에 출력해주었다.
